...
| **GeekNews** | 개발·기술·스타트업 큐레이션 커뮤니티. `news.geeknews.pe.kr`는 연결 오류가 있어 `news.hada.io`를 사용한다. | 홈·최신글에서 AI 키워드가 있는 토픽을 찾고 `/topic?id=...` 상세 페이지를 연다. | 토픽 게시 시점과 원문 게시일을 구분한다. 요약은 GeekNews 번역을 참고하되, 제품 수치와 원문은 외부 링크에서 대조한다. | Kimi K3, Chrome, AI 개발 관련 커뮤니티 선별에 사용 |
...
| **Google AI Blog (The Keyword)** | Google의 공식 AI·제품·보안·연구 발표 채널이다. AI 허브 `/innovation-and-ai/technology/ai/`, Gemini 모델 카테고리, `/security/`를 사용한다. | 홈페이지와 AI 허브를 먼저 확인한 뒤 `https://blog.google/rss/`, AI RSS(`/innovation-and-ai/technology/ai/rss/`), Gemini 모델 RSS, `news-sitemap.xml`을 병렬로 확인한다. 후보는 개별 기사 페이지에서 본문과 날짜를 다시 확인한다. | 개별 페이지의 `Aug 13, 2026` 형식 날짜를 우선한다. RSS의 `pubDate`는 후보 선별에, `news-sitemap.xml`의 `news:publication_date`는 RSS보다 늦게 반영되는 최신 글의 보조 확인에 사용한다. 썸네일은 개별 페이지의 `og:image`·대표 이미지, 그다음 RSS `media:content` 순서로 확인하며 URL을 임의 조합하지 않는다. | 사용자 요청으로 접근 방식 확인 |
...
#### 4.3.1 Google AI Blog 상세 절차

`blog.google`은 정적 HTML, RSS, 뉴스 사이트맵을 함께 제공하므로 Playwright 없이도 기본적인 기간 필터링과 후보 수집이 가능하다.

1. `https://blog.google/`에서 전체 최신 글의 방향을 확인한다.
2. `https://blog.google/innovation-and-ai/technology/ai/`에서 AI 허브의 모델·제품·연구 후보를 찾는다.
3. `https://blog.google/rss/`와 AI·Gemini 카테고리 RSS를 병렬 요청해 `title`, `link`, `pubDate`, `media:content`를 수집한다.
4. RSS의 `lastBuildDate`가 종료일보다 이전이면 `https://blog.google/news-sitemap.xml`을 추가 확인한다. 사이트맵의 `news:publication_date`와 `news:title`이 기간 안인 URL을 후보에 넣는다.
5. 후보 개별 페이지에서 페이지 제목, 본문 게시일, 첫 단락, 대표 이미지 URL을 확인한다. Google 페이지에 표시되는 AI 생성 요약은 원문 요약의 근거로 사용하지 않는다.
6. Google 공식 발표의 벤치마크, 사용자 수, 가격, 성능 수치는 `Google이 밝혔다`라고 주체를 명시하고 독립 검증 수치와 구분한다.
7. AI 기능이 포함된 일반 제품 글은 AI 기능이 본문의 핵심일 때만 포함한다. 단순히 Pixel·Workspace 제품명에 AI가 언급된 글은 대표 AI 뉴스와 분리하거나 제외한다.

주의할 점:

- 목록·RSS의 날짜가 UTC 기준일 수 있으므로 개별 페이지의 게시일과 표기 시간대를 함께 기록한다.
- `news-sitemap.xml`에는 최신 글 일부만 노출되거나 날짜가 비어 있는 제품 URL이 섞일 수 있으므로 날짜가 비어 있는 항목은 본문에 넣지 않는다.
- Google 공식 블로그는 기업의 발표 채널이므로 성능·사용자 수·시장 효과를 독립적인 사실처럼 쓰지 않는다.
- Google 페이지의 `Read AI-generated summary`는 Google AI가 생성한 실험적 요약일 수 있으므로 원문 본문을 직접 읽고 요약한다.
