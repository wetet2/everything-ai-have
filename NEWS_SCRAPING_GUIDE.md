...
- **실물 URL 전수 검증 (핵심 원칙)**:
  - 가상/추측성 URL(슬러그 임의 조합, 가상 일련번호, 추측성 카테고리 경로 등) 생성을 절대 금지한다.
  - 모든 기사 링크는 실제 발행된 웹페이지여야 하며, 스크랩 완료 전 Playwright나 HTTP 요청으로 **HTTP 200 OK 응답 및 기사 본문·제목 일치 여부를 전수 검증**한다.
  - 404 에러가 나거나, 전혀 다른 기사/포털 홈으로 리다이렉트되어서는 안 된다.
  - 페이월·CAPTCHA·봇 차단(401/403)으로 일반 독자가 열 수 없는 매체(The Information, Bloomberg 등)는 공개 보도된 대체 매체(Reuters, Techstrong, The Verge, CNBC, VentureBeat, TechCrunch 등) 원문 링크로 매핑한다.
- 목록·홈페이지에서 후보 기사를 찾고, 최종 요약과 썸네일은 가능한 한 개별 기사 페이지에서 확인한다.
- 썸네일은 외부 핫링크 대신 실제 웹페이지에서 추출한 뒤 WebP/JPEG Base64 형태로 압축하여 독립 실행성을 보장한다.
- 사용자가 지정한 시작일과 종료일을 포함한 기사만 본문에 넣는다.
- 기사 수(예: 20~25건)는 고정 제한이 아니며, 기간 내 보도량과 사용자 요구에 따라 30~40건 이상으로 유연하게 조절한다.
- 신규 주요 모델 출시 시 독립 벤치마크(Artificial Analysis, Terminal-Bench, DeepSWE 등), 토큰당 비용, 컨텍스트 윈도우, 지연시간 데이터를 함께 조사하여 비교표를 작성한다.
...
- [ ] 기본 출처 목록을 먼저 확인했는가?
- [ ] **[필수] 모든 기사 링크가 실제 작동하며 브라우저에서 200 OK로 본문이 열리는지 전수 검증했는가? (404, 가상 추측 URL, 엉뚱한 이전 기사 연결 절대 금지)**
- [ ] **[필수] 기사 카드의 제목·요약이 링크된 웹페이지의 실제 본문 내용과 100% 일치하는가?**
- [ ] 페이월·봇 차단이 있는 사이트는 일반 독자가 열람 가능한 대체 언론사 링크로 매핑했는가?
- [ ] 목록 기사와 개별 기사 날짜가 일치하는가?
- [ ] 기간 밖의 원문 발표를 기간 내 신규 기사처럼 쓰지 않았는가?
- [ ] 동일 사건을 하나로 묶고 `⭐ 중요 뉴스`로 표시했는가?
- [ ] 기업 주장·언론 보도·독립 검증 결과를 구분했는가?
- [ ] 주요 모델 발표 시 독립 벤치마크(Intelligence Index, Terminal-Bench, 비용 등)를 조사해 정리했는가?
- [ ] 대표 기사마다 제목·실물 링크·간략한 내용·썸네일이 있는가?
- [ ] AI타임스 썸네일에 `thumbnail/custom` 경로를 사용하지 않았는가?
- [ ] 썸네일 이미지가 깨지지 않고 고화질 WebP/JPEG Base64로 추출되었는가?
